...
from config import DB_PATH  # единый путь к базе для всего проекта
...
def get_connection():
    ...
    # WAL защищает базу от повреждения при внезапном завершении процесса
    # и от потери последних записанных данных
...
def get_top_users_by_balance(limit=50):
    ...
    """
    conn = sqlite3.connect(DB_PATH)  # Укажи имя своего файла БД
    cursor = conn.cursor()
